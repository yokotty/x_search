...
x_search ツールを使いXの投稿を実際に検索し、一般ユーザーの反応・感想を収集・分析して日本語でマークダウンレポートを作成してください。
...
4. 収集した投稿を以下の基準で選別する：
   - **採用する**: 一般ユーザー・個人アカウントによる感想・意見・考察
   - **除外する**: 企業・製品・メディアの公式アカウント（プレスリリース共有、宣伝投稿、@PR_TIMES系、公式ハッシュタグ付き告知など）
   - 判断が難しい場合は、投稿内容が「感想・考察」であれば採用し、「告知・宣伝」であれば除外する
5. 選別後の投稿から意見・反応・論点を整理する
...
例:
  ...
  python x_search_report.py input/20260410.md --model grok-4-fast-reasoning
    ...
    parser.add_argument(
        ...
        help="使用するGrokモデル (デフォルト: grok-4.20-0309-reasoning、他: grok-4-fast-non-reasoning, grok-4-fast-reasoning)",
    )
    parser.add_argument(
        "--api-key",
